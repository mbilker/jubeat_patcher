...
static void *__cdecl mem_set(void *s, int c, size_t n)
{
    ...
    if (n == 1296) {
        ...
        log_body_info("ultimate", "d3 limit %d->%d buf %p->%p",
            *limit, new_limit, *buf, new_buf);
        ...
        *limit = new_limit;
        ...
        log_body_info("ultimate", "d3 guard captured: pixel pool initial=%u bytes (%.1f MiB), "
...
void festo_d3_guard_check(const char *context)
{
    ...
    unsigned pool_pct  = static_cast<unsigned>(
        (static_cast<uint64_t>(pool_peak) * 100) / pool_cap);

    if (pool_free < 0x100000) {
        log_body_warning("d3-guard",
            ...
            context, pool_peak, pool_cap, pool_pct, pool_free);
    } else if (pool_pct >= 80) {
        log_body_warning("d3-guard",
            "[%s] pixel pool %u%% used (peak 0x%x / cap 0x%x, %u bytes free)",
            context, pool_pct, pool_peak, pool_cap, pool_free);
    } else {
        log_body_info("d3-guard",
            "[%s] pixel pool %u%% used (peak 0x%x / cap 0x%x, %u bytes free)",
            context, pool_pct, pool_peak, pool_cap, pool_free);
    }
    ...
    // full is steady state, not an error.
    {
        ...
        if (hi > lo && hi <= 256) {
            ...
            log_body_info("d3-guard",
                ...
                context, used_slots, static_cast<unsigned>(hi - lo), lo, hi);
        }
    }
}
